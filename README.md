...
- `1fc2b44` 泛音音高修复 + 仓库瘦身：网页端权威泛音表 `D.HARMONICS`（91 条）提取为
  `site/h5/data/harmonics.json`，教授版泛音音高与 APK 对齐（梅花三弄前 5 音修正）；
  git filter-repo 剥离 805MB folk.zip 历史，`.git` 822MB→36MB。
- `861dfbe` APK 泛音变调修复：泛音播放 rate 恒为 1（每弦固定音高不随徽位变调），
  基准改为该弦七徽频率。
- `1b2bb65` 混响重做：IR 加早反射簇（11~73ms 离散反射 + 左右 ±3ms 去相关）、
  时变低通（空气吸声）、湿链 3.2kHz 低通、wet 0.32→0.9。
- `389e793` + `204a34c` APK 泛音滤波效果链 + 🎛音效面板：
  仅 APK 泛音走 highpass(f0×0.55)→lowpass(f0×k) 混合滤波（强度可调），
  教授版零处理；混响按钮升级为音效面板（干湿度/房间大小/滤波强度频率，localStorage 持久化）。
- `a89fadc` + `58fcee1` + `36b34b6` 走手音重做（不二次拨弦）：
  - `audio.js` 新增 `glide(string,toFreq,sec)` 余音变调滑音、`vibrato(string,type,sec)`
    余音揉弦（LFO 调制 playbackRate，教授/APK 通用）——右手不弹，左手在振动弦上操作
  - voice 记录 `t0`/`bufDur`，余音不足时回退拨弦；够时拉回余音音量再滑/揉
  - 「上/下X」不再是重触发：教授版「上七九」从余音上平滑滑上去
  - 落指吟/定吟走 `vibrato` 余音揉弦，不重拨
- `2c5ae9d` 同弦重触互掐：教授版不同徽位 vkey 不同导致同弦各音层层延长；
  现在拨弦掐掉该弦所有在响旧音（30ms 收掉），符合"再拨同一根弦先前振动消掉"物理规律。
  泛音↔按音不互掐（物理可行且罕见）。
- `91475d3` 走手音前置：真实演奏拨弦激发后左手**立即**开始滑（不等拍子走完）——
  调度器检测「发声事件+紧随滑音」配对，滑音提前到拨弦起音 0.18s 后，
  时长 = 拨弦余下 + 滑音自身拍子；光标高亮仍在原时间点。
- `4c6a8e5` 吟猱前置（同规则）：落指吟/定吟/落指猱等颤音类 carry 事件同样前置到
  拨弦起音 0.18s 后开始揉——等拍子走完再揉，包络已衰听不出颤音。
  揉弦时长 = 拨弦余下 + 自身拍子，覆盖整段余音。
- `0bb378b` + `4ad081e` 大厅「从此播放」：长按单个字（250ms 无拖动）绿框标记播放起点，
  点播放从该字播到曲尾；长按后拖动自动转框选（原框选逻辑保留）；
  优先级：框选 > 从此播放 > 全曲。修 bug：clearSelection 顺序导致标记状态被清空。
