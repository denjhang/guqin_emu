...
- 撮的数字歧义：有「散」组时按音组末位数字是弦号；全按音时每组末位是弦号。
  「十一/十二/十三」总是徽位名（十徽一分等罕用），不拆为「十徽+X弦」
- 节奏与减字按非空位置对齐；控制字（泛起/泛止）无声，少息占时长
...
- **无徽位 carry 规则**：擘/托/打默认散音；抹/挑/勾/剔/摘默认按音，沿用前一徽位（APK `_inferLeft` 默认"按"）
- **带起 = 散音**：归入 `openStringCarryTechnique`（抓起/带起/推出/放合），掐起/搯起/掩为按音
- **至X 合并扫弦**：散涓二+至三 → 扫2至3，「至」不另占时长
- **撮识十一/十二/十三徽**：不拆为十徽+X弦，弦号由散弦相邻推导
- **光标同步**：事件携带 `domIdx`（非空 token 全局序号），避免 ctrl/至X/反复段导致事件索引与 DOM token 错位
- **少息占时长**：休止记号按当前节奏推进时间轴，不跳过
- **徽分制**：`digitsToHui` 一=0.1/二=0.2/.../九=0.9/半=0.5，与 APK `_parseHuiDigits` 2分位系统一致
